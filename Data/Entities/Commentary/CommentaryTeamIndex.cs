...
{
    public class CommentaryTeamIndex : IIdentity, IDataConnection
    {
        private readonly Mapping.CommentaryTeamIndex _valueMapping;
        ...
        [Display(Name = "Commentary Index", Description = "The index of the commentary sound/text of the team.")]
        [Range(67, 107, ErrorMessage = "Value for {0} must be between {1} and {2}.")]
        public int CommentaryIndex { get; set; }

        public CommentaryTeamIndex(Mapping.CommentaryTeamIndex valueMapping, int id)
        {
            _valueMapping = valueMapping;

            Id = id;
            LocalResourceId = Mapping.CommentaryTeamIndex.GetLocalResourceId(Id);
